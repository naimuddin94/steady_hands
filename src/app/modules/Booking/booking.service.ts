...
const createBooking = async (user: IAuth, artistId: string, date: Date, day: string, slotId: string) => {
  // Check if the selected slot exists and is available
  const slot = await Slot.findOne({ _id: slotId, day });
  if (!slot) {
  ...
  const existingBooking = await Booking.findOne({ artist: artistId, user: user._id, date, slot: slotId });
  if (existingBooking) {
    throw new AppError(status.BAD_REQUEST, 'You have already booked for this time slot');
  }
